...
var btcswap = function btcswap(params) {
  ...
  this.reserveTicket = function (ticketId, txHash, powNonce, callback) {
    ...
    this.contract.reserveTicket.sendTransaction(ticketId, txHash, powNonce, objParam, function (err, result) {
      if (this.debug) console.log('reserveTicket result: ', result);
      if (err) {
        callback(err);
