...
var btcSwap = function btcSwap(params) {
  ...
  this.createTicket = function (btcAddress, numEther, btcPrice, success, failure) {
    ...
    if (this.debug) console.log('Call result: ', callResult, ' duration: ', durationSec);
    ...
    this.contract.createTicket.sendTransaction(addrHex, numWei, weiPerSatoshi, objParam, (function (err, result) {
      if (this.debug) console.log('createTicket result: ', result);
      if (err) {
        failure(err);
  ...
  this.watchCreateTicket = function (addrHex, numWei, weiPerSatoshi, success, failure) {
    ...
    rvalFilter.watch(function (err, res) {
      try {
        if (err) {
          failure(err);
          if (this.debug) console.log('@@@ rvalFilter err: ', err);
          return;
        }

        if (this.debug) console.log('@@@ rvalFilter res: ', res);
        ...
        if (this.debug) console.log('@@@ filter stopWatching...');
  ...
  this.claimTicket = function (ticketId, txHex, txHash, txIndex, merkleSibling, txBlockHash, success, failure) {
    ...
    var endTime = Date.now();
    var durationSec = (endTime - startTime) / 1000;
    if (this.debug) console.log('@@@@ callResult: ', callResult, ' duration: ', durationSec);

    var rval = callResult.toNumber();
    switch (rval) {
      case ticketId:
        if (this.debug) console.log('@@@@ call GOOD so now sendTx...');
    ...
    this.contract.claimTicket.sendTransaction(ticketId, txHex, txHash, txIndex, merkleSibling, txBlockHash, objParam, function (err, result) {
      if (this.debug) console.log('claimTicket result: ', result);
      if (err) {
        failure(err);
        if (this.debug) console.log('@@@ claimTicket sendtx err: ', err);
        return;
      }
      success(result);
    });
  ...
  this.reserveTicket = function (ticketId, txHash, powNonce, success, failure) {
    ...
    this.contract.reserveTicket.call(ticketId, txHash, powNonce, objParam, (function (error, result) {
      var endTime = Date.now();
      var durationSec = (endTime - startTime) / 1000;
      if (this.debug) console.log('@@@@ callResult: ', result, ' duration: ', durationSec);
      ...
      switch (rval) {
        ...
        case RESERVE_FAIL_POW:
          ...
          return;
        default:
          if (this.debug) console.log('Unexpected error rval: ', rval);
          failure('Unexpected error' + rval);
          return;
      }
      ...
      this.contract.reserveTicket.sendTransaction(ticketId, txHash, powNonce, objParam, function (err, res) {
        ...
        if (err) {
          failure(err);
          if (this.debug) console.log('@@@ reserveTicket sendtx err: ', err);
          return;
        }
        success(res);
      });
    ...
    try {
      var powPromise = new Promise((function (resolve, reject) {
        ...
        var tryPoW = (function (i) {
          ...
          strHash = ku.wordsToHexString(dst);
          bnHash = new BigNumber('0x' + strHash);
          if (this.debug) console.log('PASS', i, strHash, bnHash.toString());

          if (i >= 100000000) reject('PoW failed.');

          i += 1;
          if (bnHash.gte(bnTarget) && i < 100000000) setTimeout(tryPoW, 10, i);else resolve(i);
        }).bind(this);
...
//   });
// }
//# sourceMappingURL=btcswap.js.map
