...
var RELAY_TESTNET = "0xa9eb7eec49f39a3030baacc95111fb94192be354";
...
var btcSwap = function btcSwap(params) {
  ...
  if (typeof params.testnet === 'undefined') this.testnet = true;else this.testnet = params.testnet;
  ...
  if (this.debug) console.log('BTC network:', this.testnet ? "TESTNET" : "LIVE");
  ...
  this.createTicket = function (btcAddress, numEther, btcTotal, success, completed, failure) {
    ...
    this.contract.createTicket.call(addrHex, numWei, weiPerSatoshi, objParam, (function (error, result) {
      ...
      this.contract.createTicket.sendTransaction(addrHex, numWei, weiPerSatoshi, objParam, (function (err, res) {
        ...
        if (err) {
          failure(err.message);
          if (this.debug) console.error('createTicket sendTx error:', err);
          return;
        }
  ...
  this.watchCreateTicket = function (pendingHash, completed, failure) {
    ...
    if (this.debug) console.log('watching for createTicket');

    createFilter.watch((function (err, res) {
      try {
        if (err) {
          failure(err.message);
          if (this.debug) console.error('createFilter error:', err);
          return;
        }
        ...
        if (this.debug) console.log('id of "created" ticket: ', ticketId);

        if (ticketId > 0) {
          setTimeout((function () {
            this.lookupTicket(ticketId, (function (ticket) {
              if (this.debug) console.log('lookup of created ticket: ', ticket);

              completed(pendingHash, ticket);
            }).bind(this), (function (error) {
              if (this.debug) console.log('error lookup for created ticket:', error);

              failure('Could not lookup created ticket: ' + error);
            }).bind(this));
          }).bind(this), 1000);
        } else {
          if (this.debug) console.log('return value from failed create ticket: ', ticketId);
  ...
  this.reserveTicket = function (ticketId, txHash, powNonce, success, completed, failure) {
    ...
    this.contract.reserveTicket.call(ticketId, txHash, powNonce, objParam, (function (error, result) {
      ...
      reserveFilter.watch((function (err, res) {
        try {
          if (err) {
            if (this.debug) console.error('reserveFilter error:', err);
            failure(err.message);
            return;
          }
          ...
          if (rval === ticketId) {
            ...
            if (this.debug) console.log('Reserve ticket error: ' + rval);
      ...
      this.contract.reserveTicket.sendTransaction(ticketId, txHash, powNonce, objParam, (function (err, res) {
        ...
        if (err) {
          failure(err.message);
          if (this.debug) console.error('reserveTicket sendTx error:', err);
          return;
        }
        success({
          id: ticketId,
  ...
  this.claimTicket = function (ticketId, txHex, txHash, txIndex, merkleSibling, txBlockHash, success, completed, failure) {
    ...
    this.contract.claimTicket.call(ticketId, txHex, txHash, txIndex, merkleSibling, txBlockHash, objParam, (function (error, result) {
      ...
      claimFilter.watch((function (err, res) {
        try {
          if (err) {
            if (this.debug) console.error('claimFilter error: ', err);
            failure(err.message);
            return;
          }
      ...
      this.contract.claimTicket.sendTransaction(ticketId, txHex, txHash, txIndex, merkleSibling, txBlockHash, objParam, (function (err, res) {
        ...
        if (err) {
          failure(err, message);
          if (this.debug) console.error('claimTicket sendTx error: ', err);
          return;
        }
        success(res);
      }).bind(this));
  ...
  this.cancelTicket = function (ticketId, success, completed, failure) {
    this.contract.cancelTicket.call(ticketId, (function (error, result) {
      ...
      if (this.debug) console.log("CANCEL", result.toNumber());
      ...
      cancelFilter.watch((function (err, res) {
        try {
          if (err) {
            if (this.debug) console.error('cancelFilter error: ', err);
            failure(err.message);
            return;
          }
          ...
          var eventArgs = res.args;
          if (eventArgs.rval.toNumber() === 0) {
            if (this.debug) console.log('Ticket canceled:', ticketId);
            completed(ticketId);
          } else {
            failure('Cancel ticket error: ' + ticketId);
          }
        } finally {
          if (this.debug) console.log('cancelFilter.stopWatching()...');
          cancelFilter.stopWatching();
        }
      }).bind(this));

      this.contract.cancelTicket.sendTransaction(ticketId, (function (err, res) {
        if (this.debug) console.log("cancelTicket result: ", res);
        if (err) {
          failure(err.message);
          if (this.debug) console.error('cancelTicket sendTx error: ', err);
          return;
        }
  ...
  this.watchTickets = function (ticketEvent, failure) {
    ...
    if (this.ticketFilter) this.ticketFilter.stopWatching();
    ...
    this.ticketFilter.watch((function (err, res) {
      try {
        if (err) {
          if (this.debug) console.error('ticketFilter error: ', err);
          failure(err.message);
          return;
        }

        if (this.debug) console.log('ticketFilter result:', res);
        ...
        if (ticketId === 0 && rval > 0) {
          if (this.debug) console.log('New ticket:', rval);
          ticketEvent("new", rval);
        }
        // Ticket reserved
        else if (ticketId > 0 && ticketId === rval) {
            if (this.debug) console.log('Ticket reserved:', rval);
            ticketEvent("reserved", rval);
          }
          // Ticket claimed or canceled
          else if (ticketId > 0 && rval === 0) {
              if (this.debug) console.log('Ticket removed:', ticketId);
        ...
        if (this.debug) console.error('ticketFilter error, should maybe stopWatching()...');
  ...
  this.computePoW = function (ticketId, btcTxHash, success, failure) {
    try {
      ...
      if (this.debug) console.log('PoW source: ', powSrc);
      ...
      var tryPoW = (function (i) {
        ...
        strHash = ku.wordsToHexString(dst);
        bnHash = new BigNumber('0x' + strHash);
        if (this.debug) console.log("PASS", i, bnHash.toNumber(), bnTarget.toNumber(), "DIFF", bnHash.minus(bnTarget).toNumber());
        ...
        i += 1;
        if (bnHash.gte(bnTarget) && i < 10000) setTimeout(tryPoW, 10, i);else {
          success(i);
  ...
  this.createTransaction = function (wallet, recipient, amountBtc, etherFee, etherAddress, success, failure) {
    ...
    if (this.debug) console.log("CREATE_TRANSACTION", wallet, recipient, amount, amountBtc, etherAddress, etherFee);
    ...
    blockchain.addresses.unspents(wallet.address, (function (err, unspents) {
      ...
      if (this.debug) console.log("UNSPENTS", unspents);
      ...
      // Add inputs
      for (var o = 0; o < inputs.length; o++) tx.addInput(inputs[o].hash, inputs[o].index);
      ...
      for (var s = 0; s < inputs.length; s++) tx.sign(s, key);
      ...
      if (this.debug) console.log(signedTx);
  ...
  this.getBlockchainHead = function (success, failure) {
    this.relay.getBlockchainHead.call((function (err, res) {
      if (err) {
        ...
        if (this.debug) console.error(error, err);
  ...
  this.storeBlockHeader = function (blockHash, success, failure) {
    ...
    var req = https.request(reqOptions, (function (res) {
      if (!res || res.statusCode !== 200) {
        ...
        if (this.debug) console.error(errorMsg, res);
        failure(errorMsg);
        return;
      }
      ...
      res.on('data', (function (data) {
        ...
        if (json.status !== 'success') {
          ...
          if (this.debug) console.error(errorMsg, json);
          failure(errorMsg);
          return;
        }
        ...
        if (!data || !data.tx) {
          ...
          if (this.debug) console.error(errorMsg, data);
          failure(errorMsg);
          return;
        }
        ...
        this.relay.storeBlockHeader.call(blockHeader, options, (function (err, result) {
          if (err) {
            if (this.debug) console.error(errorMsg, err);
          ...
          if (this.debug) console.log("STORE_BLOCK_HEADER", blockNumber, blockHash, blockHeader);

          if (blockNumber) {
            this.relay.storeBlockHeader.sendTransaction(blockHeader, options, function (error, txHash) {
              if (error) {
                if (this.debug) console.error(errorMsg, error);
              ...
              if (txHash) {
                ...
                txFilter.watch(function (filterError, newBlockHash) {
                  if (filterError) {
                    if (this.debug) console.error(errorMsg, filterError);
                    failure(errorMsg + ' Filter failed: ' + String(filterError));
                    return;
                  }
                  if (!newBlockHash) return;
    ...
    req.on('error', function (e) {
      ...
      if (this.debug) console.error(errorMsg, e);
